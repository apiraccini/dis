...
IngestionServiceDep = Annotated[IngestionService | None, Depends(get_ingestion_service)]
...
async def list_documents(
    ...
    limit: int = 100,
    tag: str | None = None,
    docs: DocumentRepoDep = None,
) -> DocumentListResponse:
    rows, total = await docs.list_documents(offset=offset, limit=limit, tag=tag)  # ty: ignore[unresolved-attribute]
...
async def get_document(
    ...
    doc = await docs.get_by_id(doc_uuid)  # ty: ignore[unresolved-attribute]
    if doc is None:
...
async def delete_document(
    ...
    try:
        await service.delete_document(doc_uuid)  # ty: ignore[unresolved-attribute]
